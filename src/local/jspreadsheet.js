    ...
    var jexcel = (function (el, options) {
        ...
        */
        ...
        obj.createCell = function (i, j, value) {
            ...
            td.setAttribute('data-x', i);
            td.setAttribute('data-y', j);
            td.setAttribute('data-js-val', value);
            td.getAttribute('data-jexcel-value')
        ...
        obj.openEditor = function (cell, empty, e) {
            ...
            if (cell.classList.contains('readonly') == true) {
                ...
                if (obj.options.columns[x].editor && typeof (obj.options.columns[x].editor.openEditor) == 'function') {
                    // Custom editors
        ...
        obj.closeEditor = function (cell, save) {
            ...
            if (save == true) {
                // If custom editor
                if (obj.options.columns[x].editor && typeof obj.options.columns[x].editor.closeEditor == 'function') {
                    // Custom editor
                ...
                var search = function (item, query, index) {
                    ...
                    if (query.includes(" ")) {
                        ...
                        for (var i = 0; i < query_array.length; i++) {
                            var query_checker = false;
                            for (var j = 0; j < item.length; j++) {
                                var target_item = ("" + item[j]).toLowerCase();
                                var target_innerHTML = ("" + obj.records[index][j].innerHTML).toLowerCase();
                                if (
                                    (target_item.includes(query_array[i]) || target_innerHTML.includes(query_array[i]))
                                ) {
                                    query_checker = true;
                                    break;
                                }
                            }
                            if (query_checker === false) {
                                found_check = false;
                            }
                        }
                    } else {
                        for (var i = 0; i < item.length; i++) {
                            ...
                            //     return true;
                            // }
                            if (('' + item[i]).toLowerCase().includes(query) ||
                                ('' + obj.records[index][i].innerHTML).toLowerCase().includes(query)) {
                                found_check = true;
                            }
                        }
                    }
